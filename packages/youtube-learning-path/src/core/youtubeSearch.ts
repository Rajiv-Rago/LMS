...
/**
 * Search YouTube for videos on a topic using multiple query variants,
 * then enrich with full metadata, dedup, and filter out very short videos.
 */
export async function searchYouTubeVideos(
  ...
  return getVideoDetails(allIds, apiKey);
}

/**
 * Fetch full video details (snippet, contentDetails, statistics) for a batch of video IDs.
 * Filters out videos shorter than 2 minutes.
 */
...
/**
 * Dedup videos by videoId and filter out very short clips.
 */
export function filterAndDedup(videos: YouTubeVideo[]): YouTubeVideo[] {
  const seen = new Set<string>();
  return videos.filter((v) => {
    if (seen.has(v.videoId)) return false;
    seen.add(v.videoId);
    return v.durationSeconds >= 120;
  });
}
